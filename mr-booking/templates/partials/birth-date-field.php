...
$initial_value  = isset( $initial_value ) ? (string) $initial_value : '';
$admin_picker   = ! empty( $admin_picker );
...
$picker_attrs   = '';
if ( $admin_picker ) {
	$picker_attrs  = ' data-mrb-birth-picker="' . esc_attr( $prefix ) . '"';
	$picker_attrs .= ' data-birth-value="' . esc_attr( $initial_value ) . '"';
}
?>
<label class="mrb-field mrb-birth-field" data-field="birth_date"<?php echo $picker_attrs; // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped ?>>
	<?php if ( $show_label ) : ?>
	...
	<input type="hidden"<?php echo $input_name ? ' name="' . esc_attr( $input_name ) . '"' : ''; ?> id="<?php echo esc_attr( $hidden_id ); ?>" value="<?php echo esc_attr( $initial_value ); ?>" />
	<?php if ( $error_id ) : ?>
