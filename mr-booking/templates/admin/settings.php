...
$tabs = array(
	...
	'calendar'   => array(
		...
		'desc'  => __( 'شمسی، میلادی، فرمت ساعت و نمایش تعطیلات', 'mr-booking' ),
	...
	'account'    => array(
		'label' => __( 'حساب کاربری', 'mr-booking' ),
		'desc'  => __( 'ورود با کد پیامکی، پنل مشتری و لغو نوبت', 'mr-booking' ),
		'icon'  => 'dashicons-admin-users',
	),
	'payment'    => array(
		'label' => __( 'پرداخت', 'mr-booking' ),
		'desc'  => __( 'پیش‌پرداخت، کیف پول، درگاه زرین‌پال و قوانین رزرو', 'mr-booking' ),
			...
			<div class="mrb-settings__panel">
				...
				<div class="mrb-settings__panel-body">
					...
					<?php elseif ( 'calendar' === $tab ) : ?>
						...
						<section class="mrb-settings__section">
							<h3><?php esc_html_e( 'فرمت ساعت', 'mr-booking' ); ?></h3>
							<p class="mrb-settings__hint"><?php esc_html_e( 'این فرمت در لیست رزروها، داشبورد، تقویم و پیامک/ایمیل مشتری یکسان اعمال می‌شود.', 'mr-booking' ); ?></p>
							<div class="mrb-settings__choices">
								<label class="mrb-choice-card">
									<input type="radio" name="settings[hour_format]" value="12" <?php checked( (string) ( $settings['hour_format'] ?? '12' ), '12' ); ?> />
									<strong><?php esc_html_e( '۱۲ ساعته', 'mr-booking' ); ?></strong>
									<span><?php esc_html_e( 'مثلاً ۴:۱۵ ب.ظ', 'mr-booking' ); ?></span>
								</label>
								<label class="mrb-choice-card">
									<input type="radio" name="settings[hour_format]" value="24" <?php checked( (string) ( $settings['hour_format'] ?? '12' ), '24' ); ?> />
									<strong><?php esc_html_e( '۲۴ ساعته', 'mr-booking' ); ?></strong>
									<span><?php esc_html_e( 'مثلاً ۱۶:۱۵', 'mr-booking' ); ?></span>
								</label>
							</div>
						</section>
						<section class="mrb-settings__section">
					...
					<?php elseif ( 'account' === $tab ) :
						$login_mode   = \MRBooking\Auth\Customer_Auth::login_mode();
						$account_page = absint( $settings['account_page_id'] ?? 0 );
						$sms_ready    = ! empty( $settings['sms_enabled'] );
						$mode_cards   = array(
							'off'      => array( __( 'غیرفعال', 'mr-booking' ), __( 'فرم رزرو مثل قبل کار می‌کند؛ بدون ورود و بدون پنل مشتری.', 'mr-booking' ) ),
							'optional' => array( __( 'اختیاری', 'mr-booking' ), __( 'مشتری می‌تواند با کد پیامکی وارد شود تا اطلاعاتش پر شود، یا مثل مهمان رزرو کند.', 'mr-booking' ) ),
							'required' => array( __( 'الزامی', 'mr-booking' ), __( 'رزرو فقط بعد از تأیید شماره موبایل با کد پیامکی ممکن است.', 'mr-booking' ) ),
						);
						?>
						<?php if ( ! $sms_ready ) : ?>
							<div class="notice notice-warning inline" style="margin:0 0 16px">
								<p>
									<?php esc_html_e( 'ارسال پیامک خاموش است؛ تا فعال‌شدن آن، کد ورود برای مشتری ارسال نمی‌شود.', 'mr-booking' ); ?>
									<a href="<?php echo esc_url( admin_url( 'admin.php?page=mr-booking-settings&tab=sms' ) ); ?>"><?php esc_html_e( 'تنظیمات پیامک', 'mr-booking' ); ?></a>
								</p>
							</div>
						<?php endif; ?>

						<section class="mrb-settings__section">
							<h3><?php esc_html_e( 'ورود مشتری', 'mr-booking' ); ?></h3>
							<div class="mrb-settings__toggles mrb-mode-cards" role="radiogroup" aria-label="<?php esc_attr_e( 'حالت ورود مشتری', 'mr-booking' ); ?>">
								<?php foreach ( $mode_cards as $key => $meta ) : ?>
									<label class="mrb-switch mrb-mode-card <?php echo $login_mode === $key ? 'is-on' : ''; ?>">
										<input type="radio" name="settings[customer_login_mode]" value="<?php echo esc_attr( $key ); ?>" <?php checked( $login_mode, $key ); ?> />
										<span class="mrb-switch__copy">
											<strong><?php echo esc_html( $meta[0] ); ?></strong>
											<small><?php echo esc_html( $meta[1] ); ?></small>
										</span>
							...
							<p class="mrb-settings__hint"><?php esc_html_e( 'هر زمان می‌توانید حالت را تغییر دهید؛ حساب‌های ساخته‌شده حفظ می‌شوند. شماره موبایل بعد از تأیید، در پروفایل مشتری قفل است.', 'mr-booking' ); ?></p>
						</section>

						<section class="mrb-settings__section">
							<h3><?php esc_html_e( 'پنل مشتری', 'mr-booking' ); ?></h3>
							<div class="mrb-settings__grid">
								<label class="mrb-field">
									<span class="mrb-field__label"><?php esc_html_e( 'صفحه حساب کاربری', 'mr-booking' ); ?></span>
									<select name="settings[account_page_id]">
										<option value="0"><?php esc_html_e( '— انتخاب صفحه —', 'mr-booking' ); ?></option>
										<?php foreach ( get_pages( array( 'post_status' => 'publish' ) ) ?: array() as $page ) : ?>
											<option value="<?php echo esc_attr( (string) $page->ID ); ?>" <?php selected( $account_page, (int) $page->ID ); ?>><?php echo esc_html( $page->post_title ); ?></option>
										<?php endforeach; ?>
									</select>
									<span class="mrb-field__hint"><?php esc_html_e( 'شورت‌کد [mr_booking_account] را در این صفحه قرار دهید. لینک «حساب کاربری» در فرم رزرو به همین صفحه اشاره می‌کند.', 'mr-booking' ); ?></span>
								</label>
								<label class="mrb-field">
									<span class="mrb-field__label"><?php esc_html_e( 'رنگ‌بندی پنل مشتری', 'mr-booking' ); ?></span>
									<select name="settings[account_color_scheme]">
										<option value="admin" <?php selected( (string) ( $settings['account_color_scheme'] ?? 'admin' ), 'admin' ); ?>><?php esc_html_e( 'سبز — همرنگ پنل مدیریت افزونه', 'mr-booking' ); ?></option>
										<option value="form" <?php selected( (string) ( $settings['account_color_scheme'] ?? 'admin' ), 'form' ); ?>><?php esc_html_e( 'همرنگ فرم رزرو (تنظیمات ظاهر)', 'mr-booking' ); ?></option>
									</select>
									<span class="mrb-field__hint"><?php esc_html_e( 'فرم رزرو داخل پنل هم از همین رنگ‌بندی پیروی می‌کند.', 'mr-booking' ); ?></span>
								</label>
								<label class="mrb-field">
									<span class="mrb-field__label"><?php esc_html_e( 'مهلت لغو توسط مشتری (دقیقه)', 'mr-booking' ); ?></span>
									<input type="number" name="settings[customer_cancel_min_minutes]" min="0" step="5" value="<?php echo esc_attr( (string) (int) ( $settings['customer_cancel_min_minutes'] ?? 30 ) ); ?>" />
									<span class="mrb-field__hint"><?php esc_html_e( 'مشتری فقط وقتی می‌تواند لغو کند که حداقل این‌قدر تا شروع نوبت مانده باشد.', 'mr-booking' ); ?></span>
								</label>
							</div>
							<div class="mrb-settings__toggles" style="margin-top:12px">
								<label class="mrb-switch">
									<input type="checkbox" name="settings[account_embed_booking]" value="1" <?php checked( ! empty( $settings['account_embed_booking'] ?? 1 ) ); ?> />
									<span class="mrb-switch__ui" aria-hidden="true"></span>
									<span class="mrb-switch__copy">
										<strong><?php esc_html_e( 'رزرو نوبت داخل پنل مشتری', 'mr-booking' ); ?></strong>
										<small><?php esc_html_e( 'تب «رزرو نوبت» با فرم کامل رزرو در پنل کاربری نمایش داده می‌شود تا مشتری بدون خروج از پنل نوبت بگیرد.', 'mr-booking' ); ?></small>
									</span>
								</label>
								<label class="mrb-switch">
									<input type="checkbox" name="settings[allow_cancellation]" value="1" <?php checked( ! empty( $settings['allow_cancellation'] ) ); ?> />
									<span class="mrb-switch__ui" aria-hidden="true"></span>
									<span class="mrb-switch__copy">
										<strong><?php esc_html_e( 'اجازه لغو نوبت از پنل مشتری', 'mr-booking' ); ?></strong>
										<small><?php esc_html_e( 'با خاموش‌کردن، دکمه لغو در پنل مشتری نمایش داده نمی‌شود.', 'mr-booking' ); ?></small>
									</span>
								</label>
							</div>
						</section>
						...
						<section class="mrb-settings__section">
							<h3><?php esc_html_e( 'پیامک کد ورود', 'mr-booking' ); ?></h3>
							<label class="mrb-field">
								<span class="mrb-field__label"><?php esc_html_e( 'متن پیامک', 'mr-booking' ); ?></span>
								<textarea name="settings[tpl_sms_otp]" rows="3"><?php echo esc_textarea( (string) ( $settings['tpl_sms_otp'] ?? '' ) ); ?></textarea>
								<span class="mrb-field__hint"><?php esc_html_e( 'متغیرها: {otp_code} کد ۵ رقمی، {business_name} نام کسب‌وکار، {minutes} مدت اعتبار. کد ۵ دقیقه معتبر است و هر شماره حداکثر ۵ بار در ساعت کد می‌گیرد.', 'mr-booking' ); ?></span>
							</label>
						</section>
						...
						<section class="mrb-settings__section">
							...
							<div class="mrb-settings__grid">
								...
								<label class="mrb-field mrb-field--wide">
									<span class="mrb-field__label"><?php esc_html_e( 'نام الگوی کد ورود (Verify / Lookup)', 'mr-booking' ); ?></span>
									<input type="text" name="settings[sms_otp_template]" dir="ltr" autocomplete="off" value="<?php echo esc_attr( (string) ( $settings['sms_otp_template'] ?? '' ) ); ?>" placeholder="mrbooking-otp" />
									<span class="mrb-field__hint">
										<?php esc_html_e( 'برای کاوه‌نگار الزامی است: اپراتورها پیامک‌های حاوی کد را که از خط اشتراکی و با ارسال معمولی فرستاده شوند فیلتر می‌کنند (سرویس «ارسال شد» گزارش می‌دهد اما پیامک نمی‌رسد). در پنل کاوه‌نگار → ارسال با الگو، یک الگو با متغیر %token% بسازید، پس از تأیید نامش را اینجا وارد کنید. اگر خالی باشد از ارسال معمولی استفاده می‌شود.', 'mr-booking' ); ?>
									</span>
								</label>
								<label class="mrb-field">
					...
					<?php elseif ( 'payment' === $tab ) : ?>
						<?php
						$deposit_on   = ! empty( $settings['show_deposit'] );
						$login_mode   = \MRBooking\Auth\Customer_Auth::login_mode();
						$merchant_set = '' !== trim( (string) ( $settings['zarinpal_merchant_id'] ?? '' ) );
						?>
						<section class="mrb-settings__section">
							<h3><?php esc_html_e( 'پیش‌پرداخت', 'mr-booking' ); ?></h3>
							<p class="mrb-settings__hint"><?php esc_html_e( 'با فعال‌شدن، مشتری هنگام انتخاب خدمت مبلغ پیش‌پرداخت را می‌بیند، مرحله «پرداخت» به فرم اضافه می‌شود و قیمت اصلی فقط برای حسابداری شما می‌ماند. با خاموش‌بودن، فرم دقیقاً مثل قبل کار می‌کند.', 'mr-booking' ); ?></p>
							<div class="mrb-settings__toggles">
								<label class="mrb-switch">
									<input type="checkbox" name="settings[show_deposit]" value="1" <?php checked( $deposit_on ); ?> />
									<span class="mrb-switch__ui" aria-hidden="true"></span>
									<span class="mrb-switch__copy">
										<strong><?php esc_html_e( 'نمایش و دریافت پیش‌پرداخت', 'mr-booking' ); ?></strong>
										<small><?php esc_html_e( 'مبلغ پیش‌پرداخت هر خدمت را از منوی خدمات تعیین کنید.', 'mr-booking' ); ?></small>
									</span>
								</label>
								<label class="mrb-switch">
									<input type="checkbox" name="settings[enable_tip]" value="1" <?php checked( ! empty( $settings['enable_tip'] ) ); ?> />
									<span class="mrb-switch__ui" aria-hidden="true"></span>
									<span class="mrb-switch__copy">
										<strong><?php esc_html_e( 'فیلد انعام در مرحله پرداخت', 'mr-booking' ); ?></strong>
										<small><?php esc_html_e( 'مشتری می‌تواند مبلغی به‌عنوان انعام به پیش‌پرداخت اضافه کند.', 'mr-booking' ); ?></small>
									</span>
								</label>
							</div>
						</section>
						...
						<section class="mrb-settings__section">
							<h3><?php esc_html_e( 'روش‌های پرداخت', 'mr-booking' ); ?></h3>
							<div class="mrb-settings__toggles">
								<label class="mrb-switch">
									<input type="checkbox" name="settings[enable_wallet_payment]" value="1" <?php checked( ! empty( $settings['enable_wallet_payment'] ) ); ?> />
									<span class="mrb-switch__ui" aria-hidden="true"></span>
									<span class="mrb-switch__copy">
										<strong><?php esc_html_e( 'پرداخت با کیف پول', 'mr-booking' ); ?></strong>
										<small><?php esc_html_e( 'فقط برای مشتریانی که با حساب کاربری وارد شده‌اند. وجه لغو نوبت‌ها به همین کیف پول برمی‌گردد.', 'mr-booking' ); ?></small>
									</span>
								</label>
								<label class="mrb-switch">
									<input type="checkbox" name="settings[enable_online_payment]" value="1" <?php checked( ! empty( $settings['enable_online_payment'] ) ); ?> />
									<span class="mrb-switch__ui" aria-hidden="true"></span>
									<span class="mrb-switch__copy">
										<strong><?php esc_html_e( 'پرداخت آنلاین (درگاه)', 'mr-booking' ); ?></strong>
										<small><?php esc_html_e( 'برای فعال‌شدن، کد مرچنت درگاه را وارد کنید.', 'mr-booking' ); ?></small>
									</span>
								</label>
							</div>
							<div class="mrb-settings__grid" style="margin-top:12px">
								<label class="mrb-field">
									<span class="mrb-field__label"><?php esc_html_e( 'حداقل مبلغ افزایش موجودی کیف پول (تومان)', 'mr-booking' ); ?></span>
									<input type="number" name="settings[wallet_topup_min]" min="1000" step="1000" value="<?php echo esc_attr( (string) (int) ( $settings['wallet_topup_min'] ?? 10000 ) ); ?>" />
									<span class="mrb-field__hint"><?php esc_html_e( 'مشتری از پنل کاربری خود می‌تواند از طریق درگاه، کیف پولش را شارژ کند (نیاز به فعال‌بودن کیف پول و درگاه).', 'mr-booking' ); ?></span>
								</label>
							</div>
							<?php if ( 'off' === $login_mode ) : ?>
								<div class="notice notice-info inline" style="margin:12px 0 0">
									<p><?php esc_html_e( 'ورود مشتری غیرفعال است؛ بنابراین کیف پول در فرم رزرو قابل استفاده نیست. آن را از تنظیمات → حساب کاربری فعال کنید.', 'mr-booking' ); ?> <a href="<?php echo esc_url( admin_url( 'admin.php?page=mr-booking-settings&tab=account' ) ); ?>"><?php esc_html_e( 'تنظیمات حساب کاربری', 'mr-booking' ); ?></a></p>
								</div>
							<?php endif; ?>
						</section>

						<section class="mrb-settings__section">
							<h3><?php esc_html_e( 'درگاه پرداخت', 'mr-booking' ); ?></h3>
							<div class="mrb-settings__grid">
								<label class="mrb-field">
									<span class="mrb-field__label"><?php esc_html_e( 'درگاه', 'mr-booking' ); ?></span>
									<select name="settings[payment_gateway]">
										<option value="zarinpal" selected><?php esc_html_e( 'زرین‌پال', 'mr-booking' ); ?></option>
									</select>
								</label>
								<label class="mrb-field">
									<span class="mrb-field__label"><?php esc_html_e( 'کد مرچنت زرین‌پال', 'mr-booking' ); ?></span>
									<input type="text" name="settings[zarinpal_merchant_id]" dir="ltr" autocomplete="off" value="<?php echo esc_attr( (string) ( $settings['zarinpal_merchant_id'] ?? '' ) ); ?>" placeholder="xxxxxxxx-xxxx-xxxx-xxxx-xxxxxxxxxxxx" />
									<span class="mrb-field__hint"><?php esc_html_e( 'از پنل زرین‌پال → درگاه‌ها. مبالغ به تومان ارسال می‌شوند.', 'mr-booking' ); ?></span>
								</label>
							</div>
							<div class="mrb-settings__toggles" style="margin-top:12px">
								<label class="mrb-switch">
									<input type="checkbox" name="settings[zarinpal_sandbox]" value="1" <?php checked( ! empty( $settings['zarinpal_sandbox'] ) ); ?> />
									<span class="mrb-switch__ui" aria-hidden="true"></span>
									<span class="mrb-switch__copy">
										<strong><?php esc_html_e( 'حالت آزمایشی (Sandbox)', 'mr-booking' ); ?></strong>
										<small><?php esc_html_e( 'برای تست بدون پرداخت واقعی. قبل از انتشار خاموش کنید.', 'mr-booking' ); ?></small>
									</span>
								</label>
							</div>
							<p class="mrb-settings__hint" style="margin-top:12px">
								<?php esc_html_e( 'آدرس بازگشت (به‌صورت خودکار به درگاه ارسال می‌شود):', 'mr-booking' ); ?>
								<code dir="ltr"><?php echo esc_html( \MRBooking\Payments\Payment_Service::callback_url() ); ?></code>
							</p>
							<?php if ( $deposit_on && ! empty( $settings['enable_online_payment'] ) && ! $merchant_set ) : ?>
								<div class="notice notice-warning inline" style="margin:12px 0 0"><p><?php esc_html_e( 'کد مرچنت وارد نشده؛ گزینه پرداخت آنلاین به مشتری نمایش داده نمی‌شود.', 'mr-booking' ); ?></p></div>
							<?php endif; ?>
						</section>

						<section class="mrb-settings__section">
							<h3><?php esc_html_e( 'قوانین و شرایط رزرو', 'mr-booking' ); ?></h3>
							<p class="mrb-settings__hint"><?php esc_html_e( 'در مرحله «تأیید رزرو» زیر خلاصه رزرو نمایش داده می‌شود. سیاست لغو و قوانین کلی را اینجا توضیح دهید.', 'mr-booking' ); ?></p>
							<div class="mrb-settings__toggles">
								<label class="mrb-switch">
									<input type="checkbox" name="settings[require_terms]" value="1" <?php checked( ! empty( $settings['require_terms'] ) ); ?> />
									<span class="mrb-switch__ui" aria-hidden="true"></span>
									<span class="mrb-switch__copy">
										<strong><?php esc_html_e( 'پذیرش قوانین الزامی باشد', 'mr-booking' ); ?></strong>
										<small><?php esc_html_e( 'بدون تیک‌زدن چک‌باکس، رزرو ثبت نمی‌شود.', 'mr-booking' ); ?></small>
									</span>
								</label>
							</div>
							<div class="mrb-settings__grid" style="margin-top:12px">
								<label class="mrb-field">
									...
									<input type="text" name="settings[text_terms_title]" value="<?php echo esc_attr( (string) ( $settings['text_terms_title'] ?? '' ) ); ?>" />
								</label>
								<label class="mrb-field">
									<span class="mrb-field__label"><?php esc_html_e( 'متن کنار چک‌باکس', 'mr-booking' ); ?></span>
									<input type="text" name="settings[text_terms_accept]" value="<?php echo esc_attr( (string) ( $settings['text_terms_accept'] ?? '' ) ); ?>" />
								</label>
								<label class="mrb-field mrb-field--wide">
									<span class="mrb-field__label"><?php esc_html_e( 'متن قوانین', 'mr-booking' ); ?></span>
									<textarea name="settings[text_terms]" rows="8"><?php echo esc_textarea( (string) ( $settings['text_terms'] ?? '' ) ); ?></textarea>
									<span class="mrb-field__hint"><?php esc_html_e( 'هر خط یک پاراگراف. می‌توانید از • برای فهرست استفاده کنید.', 'mr-booking' ); ?></span>
								</label>
							</div>
						</section>

						<section class="mrb-settings__section">
							<h3><?php esc_html_e( 'متن‌های مرحله پرداخت', 'mr-booking' ); ?></h3>
							<div class="mrb-settings__grid">
								<label class="mrb-field">
									<span class="mrb-field__label"><?php esc_html_e( 'عنوان مرحله', 'mr-booking' ); ?></span>
									<input type="text" name="settings[text_step_payment]" value="<?php echo esc_attr( (string) ( $settings['text_step_payment'] ?? '' ) ); ?>" />
								</label>
								<label class="mrb-field">
									<span class="mrb-field__label"><?php esc_html_e( 'برچسب پیش‌پرداخت', 'mr-booking' ); ?></span>
									<input type="text" name="settings[text_deposit_label]" value="<?php echo esc_attr( (string) ( $settings['text_deposit_label'] ?? '' ) ); ?>" />
								</label>
								<label class="mrb-field">
									<span class="mrb-field__label"><?php esc_html_e( 'برچسب انعام', 'mr-booking' ); ?></span>
									<input type="text" name="settings[text_tip_label]" value="<?php echo esc_attr( (string) ( $settings['text_tip_label'] ?? '' ) ); ?>" />
								</label>
							</div>
						</section>
					<?php elseif ( 'premium' === $tab ) : ?>
						...
						<section class="mrb-settings__section">
							...
							<p class="mrb-settings__hint">
								<?php esc_html_e( 'با وارد کردن کد لایسنس، امکان مخفی‌کردن لینک برند (بالای فرم رزرو عمومی) فعال می‌شود.', 'mr-booking' ); ?>
							</p>
							<div class="mrb-settings__grid">
								<label class="mrb-field" style="grid-column:1/-1">
						...
						<section class="mrb-settings__section">
							...
							<div class="mrb-settings__toggles">
								<label class="mrb-switch <?php echo $premium_active ? '' : 'is-locked'; ?>">
									...
									<span class="mrb-switch__ui" aria-hidden="true"></span>
									<span class="mrb-switch__copy">
										<strong><?php esc_html_e( 'مخفی کردن لینک برند در فرم رزرو', 'mr-booking' ); ?> <em class="mrb-premium-tag"><?php esc_html_e( 'پریمیوم', 'mr-booking' ); ?></em></strong>
										<small>
											<?php
											echo $premium_active
												? esc_html__( 'لینک برند بالای فرم رزرو (شورت‌کد و المنتور) برای بازدیدکنندگان سایت نمایش داده نمی‌شود. منو و تنظیمات پنل بدون تغییر می‌مانند.', 'mr-booking' )
