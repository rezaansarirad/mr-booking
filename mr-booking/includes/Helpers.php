...
final class Helpers {
	...
	/**
	 * Parse a user-entered amount ("۱٬۵۰۰,۰۰۰", "1,500,000", "1500000") into a float.
	 */
	public static function parse_money( $raw ): float {
		$digits = preg_replace( '/[^0-9.]/', '', str_replace( '٫', '.', self::to_english_digits( (string) $raw ) ) ) ?? '';
		if ( '' === $digits ) {
			return 0.0;
		}
	...
	/**
	 * Format an amount for editable inputs: Latin digits with thousands separators, no unit.
	 */
	public static function format_money_input( float $price ): string {
	...
	/**
	 * Plugin hour display: 12 or 24.
	 */
	public static function hour_format(): string {
		$fmt = (string) Settings\Settings::get_value( 'hour_format', '12' );

		return '24' === $fmt ? '24' : '12';
	}

	/**
	 * PHP date format for clock display.
	 */
	public static function php_time_format(): string {
		return '24' === self::hour_format() ? 'H:i' : 'g:i a';
	}

	/**
	 * Format a datetime or H:i string using the plugin hour format.
	 */
	public static function format_clock( string $datetime ): string {
		if ( preg_match( '/^\d{4}-\d{2}-\d{2}/', $datetime ) ) {
			$ts = self::timestamp_from_datetime( $datetime );
			if ( $ts ) {
				return wp_date( self::php_time_format(), $ts, wp_timezone() );
			}
		}

		$time = strlen( $datetime ) >= 16 ? substr( $datetime, 11, 5 ) : substr( $datetime, 0, 5 );
		if ( preg_match( '/^(\d{1,2}):(\d{2})/', $time, $m ) ) {
			$dummy = sprintf( '2000-01-01 %02d:%02d:00', (int) $m[1], (int) $m[2] );
			$ts    = self::timestamp_from_datetime( $dummy );
			if ( $ts ) {
				return wp_date( self::php_time_format(), $ts, wp_timezone() );
			}
		}
	...
	/**
	 * Format time portion for admin display (plugin 12/24 hour setting).
	 */
	public static function format_admin_time( string $datetime ): string {
		return self::maybe_localize_digits( self::format_clock( $datetime ) );
	}
	...
	/**
	 * Format time for customer-facing messages (plugin 12/24 hour setting).
	 */
	public static function format_customer_time( string $datetime ): string {
		return self::to_persian_digits( self::format_clock( $datetime ) );
	}
	...
	/**
	 * Author / brand label shown in admin page headers and the public form.
	 */
	public static function brand_name(): string {
		/**
		 * Filter the brand label.
		 *
		 * @param string $name Brand label.
		 */
		return (string) apply_filters( 'mr_booking_brand_name', 'Reza Ansarirad' );
	}
	...
	public static function brand_url(): string {
		/**
		 * Filter the brand URL.
		 *
		 * @param string $url Absolute URL.
		 */
		return (string) apply_filters( 'mr_booking_brand_url', 'https://rezaansarirad.ir' );
	}

	/**
	 * Escaped brand anchor markup (opens in a new tab).
	 */
	public static function brand_link( string $class = '' ): string {
		return sprintf(
			'<a href="%1$s" target="_blank" rel="noopener noreferrer"%3$s>%2$s</a>',
			esc_url( self::brand_url() ),
			esc_html( self::brand_name() ),
