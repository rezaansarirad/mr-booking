...
final class Kavenegar_Provider implements Provider_Interface {
	...
	/**
	 * OTP delivery through Kavenegar "Verify / Lookup" (pre-approved template).
	 * Operators filter code-style messages sent from shared lines via sms/send,
	 * so this is the path that actually reaches the handset.
	 *
	 * @param array<string, mixed> $config
	 */
	public function send_otp( string $to, string $code, array $config = array() ): array {
		$api_key  = trim( (string) ( $config['sms_api_key'] ?? '' ) );
		$template = trim( (string) ( $config['sms_otp_template'] ?? '' ) );
		...
		$response = wp_remote_post(
			$this->api_url( $api_key, 'verify/lookup.json' ),
			array(
				'timeout' => 20,
				'body'    => array(
					'receptor' => $to,
