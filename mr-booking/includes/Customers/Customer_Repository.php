...
final class Customer_Repository {
	...
	public static function find_by_user_id( int $user_id ): ?object {
		...
		global $wpdb;
		$table = Helpers::table( 'customers' );
		$row   = $wpdb->get_row( $wpdb->prepare( "SELECT * FROM {$table} WHERE user_id = %d ORDER BY id ASC LIMIT 1", $user_id ) ); // phpcs:ignore

		return $row ?: null;
	}

	/**
	 * Attach a WordPress user to a customer profile (one-way, never overwrites an existing link).
	 */
	public static function link_user( int $customer_id, int $user_id ): bool {
		global $wpdb;
		$updated = $wpdb->update(
			Helpers::table( 'customers' ),
			array(
				'user_id'    => $user_id,
