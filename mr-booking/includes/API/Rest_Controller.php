...
final class Rest_Controller {
	...
	public function register_routes(): void {
		...
		register_rest_route(
			...
			array(
				...
				'permission_callback' => array( $this, 'customer_search_permission' ),
			)
		);
		...
		register_rest_route(
			...
			array(
				'methods'             => 'POST',
				'callback'            => array( $this, 'create_walkin_booking' ),
				'permission_callback' => array( $this, 'walkin_permission' ),
			)
		);
	}
	...
	/**
	 * Gateway returns here; we verify, then bounce the visitor to the booking page.
	 */
	public function payment_callback( WP_REST_Request $request ) {
		$url = \MRBooking\Payments\Payment_Service::handle_callback(
			(string) $request->get_param( 'Authority' ),
	...
	public function walkin_permission(): bool {
		return Helpers::user_can( Capabilities::WALKIN );
	}

	public function customer_search_permission(): bool {
		return $this->admin_permission() || $this->walkin_permission();
	}
	...
	public function search_customers( WP_REST_Request $request ): WP_REST_Response {
		...
		if ( strlen( $q ) >= 2 ) {
			$found    = Customer_Repository::query( array( 'search' => $q, 'limit' => 12, 'order' => 'ASC', 'orderby' => 'first_name' ) );
			$balances = \MRBooking\Wallet\Wallet_Repository::balances( array_map( static fn( $c ) => (int) $c->id, $found ) );
			foreach ( $found as $c ) {
				$balance = (float) ( $balances[ (int) $c->id ] ?? 0 );
				$items[] = array(
					'wallet_balance' => $balance,
					'wallet_label'   => $balance > 0 ? Helpers::format_price( $balance ) : Helpers::to_persian_digits( '0' ) . ' ' . __( 'تومان', 'mr-booking' ),
					'id'         => (int) $c->id,
					...
					'label'      => trim( $c->first_name . ' ' . $c->last_name ) . ' — ' . $c->phone
						. ' · ' . __( 'کیف پول', 'mr-booking' ) . ': ' . ( $balance > 0 ? Helpers::format_price( $balance ) : Helpers::to_persian_digits( '0' ) ),
				);
			}
		}
	...
	public function create_walkin_booking( WP_REST_Request $request ): WP_REST_Response {
		$payload = $request->get_json_params();
		if ( ! is_array( $payload ) ) {
			$payload = $request->get_params();
		}

		$result = Booking_Service::create_walkin( $payload );

		if ( empty( $result['ok'] ) ) {
			return new WP_REST_Response( $result, 400 );
		}

		return new WP_REST_Response( $this->format_booking_response( $result, __( 'مراجعه حضوری ثبت شد.', 'mr-booking' ) ), 200 );
	}
	...
	private function format_booking_response( array $result, ?string $message = null ): array {
		...
		// Admins always get the amount (accounting); visitors only when prices are public.
		if ( $show_prices || $is_admin ) {
		...
		if ( (float) ( $booking->paid_amount ?? 0 ) > 0 ) {
			$payload['paid_label']    = Helpers::format_price( (float) $booking->paid_amount );
			$payload['deposit_label'] = Helpers::format_price( (float) ( $booking->deposit_amount ?? 0 ) );
			$payload['tip_label']     = Helpers::format_price( (float) ( $booking->tip_amount ?? 0 ) );
		}
	...
	public function get_services( WP_REST_Request $request ): WP_REST_Response {
		...
		$show_prices_global = ! $deposit_mode && ! empty( Settings::get_value( 'show_prices' ) );
		...
		foreach ( $list as $s ) {
			...
			if ( $deposit_mode ) {
				$item['deposit']       = (float) ( $s->deposit ?? 0 );
				$item['deposit_label'] = Helpers::format_price( (float) ( $s->deposit ?? 0 ) );
			}
	...
	public function create_booking( WP_REST_Request $request ): WP_REST_Response {
		...
		$booking    = $result['booking'];
		$due        = (float) ( $booking->deposit_amount ?? 0 ) + (float) ( $booking->tip_amount ?? 0 );

		if ( $due > 0 ) {
			$method     = sanitize_key( (string) ( $payload['payment_method'] ?? '' ) );
			$return_url = esc_url_raw( (string) ( $payload['return_url'] ?? '' ) );
			if ( ! $return_url || 0 !== strpos( $return_url, home_url() ) ) {
				$return_url = home_url( '/' );
			}

			$payment = \MRBooking\Payments\Payment_Service::start( $booking_id, $method, $return_url );

			if ( empty( $payment['ok'] ) ) {
				// Nothing was charged; release the slot so the visitor can retry.
				\MRBooking\Bookings\Booking_Repository::delete_with_logs( $booking_id );
				return new WP_REST_Response( array( 'ok' => false, 'error' => $payment['error'] ?? __( 'پرداخت انجام نشد.', 'mr-booking' ) ), 400 );
			}

			if ( ! empty( $payment['redirect'] ) ) {
				return new WP_REST_Response( array( 'ok' => true, 'redirect' => $payment['redirect'] ), 200 );
			}

			$result['booking'] = \MRBooking\Bookings\Booking_Repository::find( $booking_id );
		}
	...
	public function public_settings(): WP_REST_Response {
		...
		return new WP_REST_Response(
			array(
				...
				'hour_format'            => Helpers::hour_format(),
				...
				'colors'                 => array(
					...
					'input_placeholder' => $s['color_input_placeholder'] ?? '#94A3B8',
					'radio_active'  => $s['color_radio_active'] ?? $s['color_primary'],
					'service_text'  => $s['color_service_text'] ?? $s['color_label'] ?? $s['color_text'],
					'service_desc'  => $s['color_service_desc'] ?? '#64748B',
					'service_card'  => $s['color_service_card'] ?? $s['color_card'],
					'service_card_selected' => $s['color_service_card_selected'] ?? '#E8F5F3',
					'service_card_border'   => $s['color_service_card_border'] ?? $s['color_border'],
					'service_duration_bg'   => $s['color_service_duration_bg'] ?? '#E8F5F1',
					'service_duration_text' => $s['color_service_duration_text'] ?? $s['color_primary'],
					'service_duration_bg_selected' => $s['color_service_duration_bg_selected'] ?? '#D1EDE8',
					'service_price'         => $s['color_service_price'] ?? $s['color_accent'],
					'service_check_border'  => $s['color_service_check_border'] ?? $s['color_service_card_border'] ?? $s['color_border'],
					'service_check_active'  => $s['color_service_check_active'] ?? $s['color_primary'],
				...
				'texts'                  => array(
					...
					'ph_first_name'      => $s['text_ph_first_name'],
					'ph_last_name'       => $s['text_ph_last_name'],
					'ph_phone'           => $s['text_ph_phone'],
					'ph_email'           => $s['text_ph_email'],
					'ph_birth_date'      => $s['text_ph_birth_date'],
					'ph_booking_for_name'=> $s['text_ph_booking_for_name'],
					'ph_staff'           => $s['text_ph_staff'],
				),
