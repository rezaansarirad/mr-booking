...
final class Settings {
	...
	public static function init(): void {
		...
		// Schema changes must land before the first frontend/REST write, not only on an admin visit.
		if ( get_option( 'mr_booking_db_version' ) !== MR_BOOKING_DB_VERSION ) {
	...
	public static function defaults(): array {
		$defaults = array(
			...
			'customer_login_mode'         => 'off', // off | optional | required
			'customer_cancel_min_minutes' => 30,
			'account_page_id'             => 0,

			// Deposits, payments, wallet, terms.
			'show_deposit'            => 0,
			'enable_tip'              => 1,
			'enable_wallet_payment'   => 1,
			'enable_online_payment'   => 1,
			'payment_gateway'         => 'zarinpal',
			'zarinpal_merchant_id'    => '',
			'zarinpal_sandbox'        => 0,
			'require_terms'           => 1,
			'wallet_topup_min'        => 10000,
			'account_embed_booking'   => 1,
			'account_color_scheme'    => 'admin', // admin | form
		...
		return array_merge(
			...
			'text_step_payment'       => 'پرداخت',
			'text_deposit_label'      => 'پیش‌پرداخت',
			'text_tip_label'          => 'انعام (اختیاری)',
			'text_terms_title'        => 'قوانین و شرایط رزرو',
			'text_terms'              => "با ثبت این رزرو، قوانین زیر را می‌پذیرید:\n• لغو نوبت فقط تا مهلت تعیین‌شده قبل از زمان نوبت ممکن است. در صورت لغو به‌موقع، پیش‌پرداخت به کیف پول شما بازگردانده می‌شود.\n• در صورت عدم حضور یا لغو دیرهنگام، پیش‌پرداخت قابل بازگشت نیست.\n• لطفاً ۱۰ دقیقه قبل از زمان نوبت حاضر باشید.",
			'text_terms_accept'       => 'قوانین و شرایط رزرو را خوانده‌ام و می‌پذیرم.',
			...
			'tpl_sms_otp'               => 'کد ورود شما به {business_name}: {otp_code}
