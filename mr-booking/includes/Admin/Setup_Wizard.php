...
final class Setup_Wizard {
	...
	public function assets( string $hook ): void {
		...
		wp_enqueue_script(
			...
			array( 'mr-booking-color-input', 'mr-booking-money-input' ),
			MR_BOOKING_VERSION,
			true
		);
	}
	...
	private function save_calendar(): void {
		...
		$hour_format = sanitize_text_field( wp_unslash( $_POST['hour_format'] ?? '12' ) );
		if ( ! in_array( $hour_format, array( '12', '24' ), true ) ) {
			$hour_format = '12';
		}
	...
	private function save_services(): void {
		...
		foreach ( $names as $i => $name ) {
			...
			$price = Helpers::parse_money( (string) ( $prices[ $i ] ?? '' ) );
			Service_Repository::save(
				array(
					'name'      => $name,
