...
final class Plugin {
	...
	public function boot(): void {
		...
		( new Shortcode() )->hooks();
		( new \MRBooking\Frontend\Account_Shortcode() )->hooks();
		( new Frontend_Assets() )->hooks();
		( new Rest_Controller() )->hooks();
		( new \MRBooking\API\Account_Controller() )->hooks();
		( new \MRBooking\Auth\Customer_Auth() )->hooks();
		( new Notification_Service() )->hooks();
		( new \MRBooking\Payments\Payment_Service() )->hooks();
		( new Privacy() )->hooks();
