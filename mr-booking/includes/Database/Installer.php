...
final class Installer {
	...
	public static function migrate_columns(): void {
		...
		// 1.3.0 — deposits, payments, wallet.
		// phpcs:ignore WordPress.DB.PreparedSQL.InterpolatedNotPrepared
		if ( empty( $wpdb->get_results( "SHOW COLUMNS FROM {$table} LIKE 'deposit'" ) ) ) {
			// phpcs:ignore WordPress.DB.PreparedSQL.InterpolatedNotPrepared
			$wpdb->query( "ALTER TABLE {$table} ADD COLUMN deposit DECIMAL(12,2) NOT NULL DEFAULT 0 AFTER price" );
		}

		if ( ! get_option( 'mr_booking_deposit_defaults_applied' ) ) {
			// phpcs:ignore WordPress.DB.PreparedSQL.InterpolatedNotPrepared
			$wpdb->query( "UPDATE {$table} SET deposit = ROUND(price * 0.5 / 1000) * 1000 WHERE has_price = 1 AND price > 0 AND deposit = 0" );
			update_option( 'mr_booking_deposit_defaults_applied', 1, false );
		}

		$bookings_table  = Helpers::table( 'bookings' );
		$booking_columns = array(
			'deposit_amount' => "DECIMAL(12,2) NOT NULL DEFAULT 0 AFTER total_price",
			'tip_amount'     => "DECIMAL(12,2) NOT NULL DEFAULT 0 AFTER deposit_amount",
			'paid_amount'    => "DECIMAL(12,2) NOT NULL DEFAULT 0 AFTER tip_amount",
			'payment_status' => "VARCHAR(20) NOT NULL DEFAULT 'none' AFTER paid_amount",
			'payment_method' => "VARCHAR(20) NULL AFTER payment_status",
			'payment_ref'    => "VARCHAR(100) NULL AFTER payment_method",
			'terms_accepted' => "TINYINT(1) NOT NULL DEFAULT 0 AFTER payment_ref",
		);
		foreach ( $booking_columns as $column => $definition ) {
			// phpcs:ignore WordPress.DB.PreparedSQL.InterpolatedNotPrepared
			if ( empty( $wpdb->get_results( "SHOW COLUMNS FROM {$bookings_table} LIKE '{$column}'" ) ) ) {
				// phpcs:ignore WordPress.DB.PreparedSQL.InterpolatedNotPrepared
				$wpdb->query( "ALTER TABLE {$bookings_table} ADD COLUMN {$column} {$definition}" );
			}
		}

		$wallet_table = Helpers::table( 'wallet_transactions' );
		if ( $wallet_table !== $wpdb->get_var( $wpdb->prepare( 'SHOW TABLES LIKE %s', $wallet_table ) ) ) {
			require_once ABSPATH . 'wp-admin/includes/upgrade.php';
			dbDelta(
