...
final class Booking_Repository {
	...
	/**
	 * Source value for walk-in bookings (served on the spot; never blocks a slot).
	 */
	public const SOURCE_WALKIN = 'walkin';

	/**
	 * Human labels for booking sources.
	 *
	 * @return array<string, string>
	 */
	public static function source_labels(): array {
		return array(
			'frontend'          => __( 'آنلاین', 'mr-booking' ),
			'phone'             => __( 'تلفنی', 'mr-booking' ),
			self::SOURCE_WALKIN => __( 'حضوری', 'mr-booking' ),
		);
	}

	public static function is_walkin( object $booking ): bool {
		return self::SOURCE_WALKIN === (string) ( $booking->source ?? '' );
	}
	...
	public static function query( array $args = array() ): array {
		...
		$wt = Helpers::table( 'wallet_transactions' );
		...
		if ( ! empty( $args['source'] ) ) {
			$where[]  = 'b.source = %s';
			$params[] = sanitize_key( (string) $args['source'] );
		}
		...
		if ( ! empty( $args['datetime_from'] ) ) {
			$where[]  = 'b.start_datetime >= %s';
			$params[] = sanitize_text_field( (string) $args['datetime_from'] );
		}
		...
		if ( 'nearest' === $orderby_raw ) {
			$order_sql = 'CASE WHEN b.start_datetime >= %s THEN 0 ELSE 1 END ASC, b.start_datetime ASC';
			$params[]  = current_time( 'mysql' );
		} else {
			$orderby = in_array( $orderby_raw, array( 'created_at', 'start_datetime', 'id' ), true )
				? $orderby_raw
				: 'start_datetime';
			$order_sql = "b.{$orderby} {$order}";
		}

		$sql = "SELECT b.*, c.first_name, c.last_name, c.phone, c.email,
				TRIM(CONCAT_WS(' ', st.first_name, st.last_name)) AS staff_name,
				(SELECT COALESCE(SUM(w.amount),0) FROM {$wt} w WHERE w.customer_id = b.customer_id) AS wallet_balance,
				(
	...
	public static function find( int $id ): ?object {
		...
		$row = $wpdb->get_row(
			$wpdb->prepare(
				"SELECT b.*, c.id AS joined_customer_id, c.first_name, c.last_name, c.phone, c.email, c.birth_date, c.notes AS customer_notes, b.customer_id
	 ...
	 */
	public static function blocks_slot( string $status, ?object $booking = null ): bool {
		if ( $booking && self::is_walkin( $booking ) ) {
			return false;
		}
	...
	public static function overlapping( string $start, string $end, ?int $staff_id = null, int $exclude_id = 0, int $padding_minutes = 0 ): array {
		...
		$params = array_merge( $blocking, array( self::SOURCE_WALKIN, $end, $start ) );
	...
	public static function for_date( string $ymd, ?int $staff_id = null ): array {
		...
		$params = array_merge( array( $ymd ), $blocking, array( self::SOURCE_WALKIN ) );
	...
	public static function create( array $data, array $services ): int {
		...
		$row = array(
			...
			'paid_amount'      => 0,
			'payment_status'   => sanitize_key( (string) ( $data['payment_status'] ?? 'none' ) ),
